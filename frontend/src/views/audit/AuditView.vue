...
// 键与后端账本真实 event_type 一致（preparations/ledger 写入的口径）
const EVENT_META: Record<string, { label: string; color: string }> = {
  prepare: { label: '生成准备单', color: 'var(--cyan)' },
  approve: { label: '审批通过', color: 'var(--green)' },
  reject: { label: '审批拒绝', color: 'var(--red)' },
  commit: { label: '提交执行', color: 'var(--green)' },
  dry_run: { label: 'Dry-Run', color: 'var(--violet)' },
  cancel: { label: '运行取消', color: 'var(--txt-2)' },
  rollback: { label: '受管回滚', color: 'var(--amber)' },
  rerun: { label: '安全重跑', color: 'var(--cyan)' },
  prepare_expired: { label: '准备单过期', color: 'var(--txt-2)' },
  execute_pipeline_executed: { label: '执行完成', color: 'var(--green)' },
  execute_pipeline_failed: { label: '执行失败', color: 'var(--red)' },
  dry_run_executed: { label: '试运行完成', color: 'var(--violet)' },
  rollback_executed: { label: '回滚完成', color: 'var(--amber)' },
  token_issue: { label: '令牌签发', color: 'var(--green)' },
  config_change: { label: '配置变更', color: 'var(--amber)' },
}
